...
func Get_openapi_typejsonyamlHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		url := fmt.Sprintf("%s/openapi.%s", cfg.BaseURL)
...
func CreateGet_openapi_typejsonyamlTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_openapi_type_json|yaml",
		mcp.WithDescription("Get openapi.{type:json|yaml}"),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Get_openapi_typejsonyamlHandler(cfg),
	}
}
