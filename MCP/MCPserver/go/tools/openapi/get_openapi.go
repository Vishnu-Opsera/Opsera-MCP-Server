...
func Get_openapiHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		url := fmt.Sprintf("%s/openapi", cfg.BaseURL)
...
func CreateGet_openapiTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_openapi",
		mcp.WithDescription("Get openapi"),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Get_openapiHandler(cfg),
	}
}
